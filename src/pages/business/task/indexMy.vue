...
export default defineComponent({
  components: {
    TaskFormThree, TaskFormFive, UserRate,
  },
  setup() {
    ...
    const fetchTasks = async () => {
      const { tasks: data, total: count } = await TaskService.fetchTasksMy(
        currentPage.value,
        pageSize,
    ...
    <el-tabs type="border-card" m-3 flex-1 overflow-auto>
      <el-tab-pane label="任务管理">
          ...
          <el-table-column label="操作">
            <template #default="{ row }">
              <el-button-group>
                ...
                <el-button v-if="row.taskReceiverId == user.userInfo.id && row.taskState == '2'" type="primary" size="small" @click="handleEdit(row)">
                  完工
                </el-button>
              </el-button-group>
            </template>
        ...
        <el-dialog
          ...
          destroy-on-close="true"
        >
          <TaskFormFive :task="selectedTask" :is-editting="false" @submit="handleUpdate" @cancel="handleCancelView" />
        </el-dialog>

        <el-dialog
          v-model="editDialogVisible" :close-on-click-modal="false" class="!w-2xl" draggable title="完成任务"
          destroy-on-close="true"
        >
          <TaskFormThree :task="selectedTask" :is-editting="true" @submit="handleUpdate" @cancel="handleCancelEdit" />
        </el-dialog>
      </el-tab-pane>
    </el-tabs>
  </div>
